...
public class KPassabilityLayerSubtractNodeTests extends KStandardTestClass {
    ...
    public void testProcessSuccess() {
        ...
        p2t.setState(2, 0, KPassabilityState.VOID);

        p1.refresh();
        p2.refresh();

        KPassabilityLayerSubtractNode node = new KPassabilityLayerSubtractNode();
        ...
        Assertions.assertEquals(new KSize(3, 3), result.getSize());

        for (int x = 1; x < 3; x++) {
            for (int y = 0; y < 3; y++) {
                Assertions.assertEquals(KPassabilityState.VOID, result.getOnPosition(x, y));
            }
        }

        Assertions.assertEquals(KPassabilityState.IMPASSABLE, result.getOnPosition(0, 0));
        Assertions.assertEquals(KPassabilityState.IMPASSABLE, result.getOnPosition(0, 1));
        Assertions.assertEquals(KPassabilityState.IMPASSABLE, result.getOnPosition(0, 2));
    }
