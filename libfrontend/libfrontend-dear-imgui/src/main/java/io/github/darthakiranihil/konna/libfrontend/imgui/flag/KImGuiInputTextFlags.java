 ...
 * @since 0.1.0
 */
public final class KImGuiInputTextFlags {
    /**
    ...
    /**
     * Escape key clears content if not empty, and deactivate otherwise (contrast to default
    ...
    /**
     * In multi-line mode, validate with Enter, add new line with Ctrl+Enter (default is opposite:
    ...
    /**
     * Disable undo/redo. Note that input text owns the text data while active, if you want to
    ...
    /**
     * Callback on character inputs to replace or discard them. Modify 'EventChar' to replace or
    ...
    /**
     * Callback on buffer capacity changes request (beyond 'buf_size' parameter value), allowing the
     * string to grow. Notify when the string wants to be resized (for string types which hold a
     * cache of their Size). You will be provided a new BufSize in the callback and NEED to honor
    ...
    /**
     * Callback on any edit (note that InputText() already returns true on edit, the callback is
