...
/**
 * Flags for ImGui::BeginTable() [BETA API] API may evolve slightly! If you use this, please update
 * to the next version when it comes out! - Important! Sizing policies have complex and subtle side
 * effects, more so than you would expect. Read comments/demos carefully + experiment with live
 * demos to get acquainted with them. - The DEFAULT sizing policies are: - Default to SizingFixedFit
 *    if ScrollX is on, or if host window has ImGuiWindowFlags_AlwaysAutoResize. - Default to
 * SizingStretchSame if ScrollX is off. - When ScrollX is off: - Table defaults to SizingStretchSame
 * {@code ->} all Columns defaults to ImGuiTableColumnFlags_WidthStretch with same weight. - Columns
 * sizing policy allowed: Stretch (default), Fixed/Auto. - Fixed Columns will generally obtain their
 * requested width (unless the table cannot fit them all). - Stretch Columns will share the
 * remaining width. - Mixed Fixed/Stretch columns is possible but has various side-effects on
 * resizing behaviors. The typical use of mixing sizing policies is: any number of LEADING Fixed
 * columns, followed by one or two TRAILING Stretch columns. (this is because the visible order of
 * columns have subtle but necessary effects on how they react to manual resizing). - When ScrollX
 * is on: - Table defaults to SizingFixedFit {@code ->} all Columns defaults to
 * ImGuiTableColumnFlags_WidthFixed - Columns sizing policy allowed: Fixed/Auto mostly. - Fixed
 * Columns can be enlarged as needed. Table will show an horizontal scrollbar if needed. - When
 * using auto-resizing (non-resizable) fixed columns, querying the content width to use item
 ...
 * value for 'inner_width' in BeginTable(). If you specify a value for 'inner_width' then
 * effectively the scrolling space is known and Stretch or mixed Fixed/Stretch columns become
...
public final class KImGuiTableFlags {
    ...
    /**
     * Enable reordering columns in header row (need calling TableSetupColumn() + TableHeadersRow()
     * to display headers)
     *
    ...
    /**
     * Right-click on columns body/contents will display table context menu. By default it is
    ...
    /**
     * Set each RowBg color with ImGuiCol_TableRowBg or ImGuiCol_TableRowBgAlt (equivalent of
     ...
     * {@code ->} May move to style
     *
     * <p>Definition: {@code 1 << 11}
     */
    public static final int NoBordersInBody = 2048;
    /**
     * [ALPHA] Disable vertical borders in columns Body until hovered for resize (borders will
     * always appear in Headers). {@code ->} May move to style
     *
     * <p>Definition: {@code 1 << 12}
     */
    public static final int NoBordersInBodyUntilResize = 4096;
    /**
     * Columns default to _WidthFixed or _WidthAuto (if resizable or not resizable), matching
     * contents width.
     *
    ...
    /**
     * Columns default to _WidthFixed or _WidthAuto (if resizable or not resizable), matching the
     * maximum contents width of all columns. Implicitly enable
    ...
    /**
     * Columns default to _WidthStretch with default weights proportional to each columns contents
     * widths.
     *
    ...
    /**
     * Columns default to _WidthStretch with default weights all equal, unless overridden by
     * TableSetupColumn().
     *
     ...
     * Only available when ScrollX/ScrollY are disabled. Data below the limit will be clipped and
     * not visible.
     *
    ...
    /**
     * Disable clipping rectangle for every individual columns (reduce draw command count, items
     * will be able to overflow into other columns). Generally incompatible with
    ...
    /**
     * Default if BordersOuterV is on. Enable outermost padding. Generally desirable if you have
     * headers.
     *
    ...
    /**
     * Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable() to specify the
    ...
    /**
     * Hold shift when clicking headers to sort on multiple column. TableGetSortSpecs() may return
     * specs where (SpecsCount{@code >}1).
     *
