 ...
 * values. All our named keys are {@code >=} 512. Keys value 0 to 511 are left unused as legacy
 * native/opaque key values ({@code <} 1.87). Since {@code >=} 1.89 we increased typing (went from
 * int to enum), some legacy code may need a cast to ImGuiKey. Read details about the 1.87 and 1.89
 * transition : https://github.com/ocornut/imgui/issues/4921 Note that "Keys" related to physical
 * keys and are not the same concept as input "Characters", the later are submitted via
 * io.AddInputCharacter().
 *
 * @author Darth Akira Nihil
 * @since 0.1.0
 */
public final class KImGuiKey {
    /**
     * Keyboard
     *
     * <p>Definition: {@code 0}
    ...
    /**
     * X (Xbox)         Y (Switch)   Square (PS)        // Tap: Toggle Menu. Hold: Windowing mode
    ...
    /**
     * D-pad Left                                       // Move / Tweak / Resize Window (in
     * Windowing mode)
     */
    public static final int GamepadDpadLeft = 637;
    /**
     * D-pad Right                                      // Move / Tweak / Resize Window (in
     * Windowing mode)
     */
    public static final int GamepadDpadRight = 638;
    /**
     * D-pad Up                                         // Move / Tweak / Resize Window (in
     * Windowing mode)
     */
    public static final int GamepadDpadUp = 639;
    /**
     * D-pad Down                                       // Move / Tweak / Resize Window (in
     * Windowing mode)
     */
    public static final int GamepadDpadDown = 640;
    /**
     * L Bumper (Xbox)  L (Switch)   L1 (PS)            // Tweak Slower / Focus Previous (in
     * Windowing mode)
     */
    public static final int GamepadL1 = 641;
    /**
     * R Bumper (Xbox)  R (Switch)   R1 (PS)            // Tweak Faster / Focus Next (in Windowing
     * mode)
     */
    ...
    /**
     * Aliases: Mouse Buttons (auto-submitted from AddMouseButtonEvent() calls) - This is mirroring
     * the data also written to io.MouseDown[], io.MouseWheel, in a format allowing them to be
    ...
    /**
     * Keyboard Modifiers (explicitly submitted by backend via AddKeyEvent() calls) - This is
     * mirroring the data also written to io.KeyCtrl, io.KeyShift, io.KeyAlt, io.KeySuper, in a
     * format allowing them to be accessed via standard key API, allowing calls such as
     * IsKeyPressed(), IsKeyReleased(), querying duration etc. - Code polling every key (e.g. an
     * interface to detect a key press for input mapping) might want to ignore those and prefer
     * using the real keys (e.g. ImGuiKey_LeftCtrl, ImGuiKey_RightCtrl instead of ImGuiMod_Ctrl). -
     * In theory the value of keyboard modifiers should be roughly equivalent to a logical or of the
     * equivalent left/right keys. In practice: it's complicated; mods are often provided from
     * different sources. Keyboard layout, IME, sticky keys and backends tend to interfere and break
     * that equivalence. The safer decision is to relay that ambiguity down to the end-user... - On
     * macOS, we swap Cmd(Super) and Ctrl keys at the time of the io.AddKeyEvent() call.
     *
     * <p>Definition: {@code 0}
     */
