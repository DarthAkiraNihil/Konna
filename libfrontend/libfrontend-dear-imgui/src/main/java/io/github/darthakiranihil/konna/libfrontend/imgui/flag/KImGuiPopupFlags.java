...
/**
 * Flags for OpenPopup*(), BeginPopupContext*(), IsPopupOpen() functions. - To be backward
 * compatible with older API which took an 'int mouse_button = 1' argument, we need to treat small
 * flags values as a mouse button index, so we encode the mouse button in the first few bits of the
 ...
 * BeginPopupContextXXX functions to 1 instead of 0. - Multiple buttons currently cannot be
...
public final class KImGuiPopupFlags {
    ...
    /**
     * For OpenPopup*(), BeginPopupContext*(): don't reopen same popup if already open (won't
    ...
    /**
     * For IsPopupOpen(): search/test at any level of the popup stack (default test in the current
     * level)
     *
