...
/**
 * Flags for ImGui::DockSpace(), shared/inherited by child nodes (Some flags can be applied to
 * individual nodes directly).
 *
 * @author Darth Akira Nihil
 * @since 0.1.0
 */
public final class KImGuiDockNodeFlags {
    /**
     * Definition: {@code 0}
     */
    ...
    /**
     * Enable passthru dockspace: 1) DockSpace() will render a ImGuiCol_WindowBg background covering
     * everything excepted the Central Node when empty. Meaning the host window should probably use
     * SetNextWindowBgAlpha(0.0f) prior to Begin() when using this. 2) When Central Node is empty:
     * let inputs pass-through + won't display a DockingEmptyBg background. See demo for details.
     *
