...
/**
 * Flags for ImGui::IsItemHovered(), ImGui::IsWindowHovered() Note:if you are trying to check
 * whether your mouse should be dispatched to Dear ImGui or to your app, you should
 * use'io.WantCaptureMouse'instead!Please read the FAQ!Note: windows with the
...
public final class KImGuiHoveredFlags {
    ...
    /**
     * IsItemHovered() only: Disable using gamepad/keyboard navigation state when active, always
     * query mouse
     *
    ...
    /**
     * Definition:
     * {@code ImGuiHoveredFlags_AllowWhenOverlappedByItem |
     * ImGuiHoveredFlags_AllowWhenOverlappedByWindow}
     */
    public static final int AllowWhenOverlapped = 768;
    /**
     * Definition:
     * {@code ImGuiHoveredFlags_AllowWhenBlockedByPopup |
    ...
    /**
     * Require mouse to be stationary for style.HoverStationaryDelay (~0.15 sec) _at least one
     * time_. After this, can move on same item/window. Using the stationary test tends to reduces
     * the need for a long delay.
     *
     * <p>Definition: {@code 1 << 13}
     */
    public static final int Stationary = 8192;
    /**
     * IsItemHovered() only: Return true immediately (default). As this is the default you generally
     * ignore this.
     *
    ...
    /**
     * IsItemHovered() only: Return true after style.HoverDelayShort elapsed (~0.15 sec) (shared
     * between items) + requires mouse to be stationary for style.HoverStationaryDelay (once per
     * item).
     *
    ...
    /**
     * IsItemHovered() only: Return true after style.HoverDelayNormal elapsed (~0.40 sec) (shared
     * between items) + requires mouse to be stationary for style.HoverStationaryDelay (once per
     * item).
     *
