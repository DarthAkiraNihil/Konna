 ...
 * a wrapper of {@link HashMap}(String, Object)
 * with some methods that simplify working with the container.
 *
 * @since 0.1.0
...
public class KUniversalMap extends HashMap<String, Object> {
     ...
     * There is no checking of class equality so {@link ClassCastException}
     * will be thrown if you try to get value of incorrect type. Also, all standard Map.get()
     * exceptions also could be thrown. This method does not work with primitive
     * types' classes (like int.class), so use boxed classes for it
     ...
     * This method does not work with primitive types' classes (like int.class),
     * so use boxed classes for it
     * @param key The key of the value
     * @param clazz Class to cast the value
     * @return Map's value with passed key cast to required type
     * @param <T> The type to cast the value
